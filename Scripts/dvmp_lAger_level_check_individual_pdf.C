//============================================================
// Generator-level lAger check for DVMP J/#psi
//
// This macro saves:
//   1. The original combined summary canvases
//   2. Each individual histogram as a separate PDF
//============================================================

#include <iostream>

#include "TChain.h"
#include "TCanvas.h"
#include "TH1F.h"
#include "TLegend.h"
#include "TLorentzVector.h"
#include "TString.h"
#include "TPRegexp.h"
#include "TObjArray.h"
#include "TObjString.h"
#include "TSystem.h"
#include "TStyle.h"
#include "TROOT.h"
#include "TMath.h"

//============================================================
// Helper function: add legend to any histogram plot
//============================================================
void AddTruthLegend(TH1 *h,
                    const TString &lumiLabel,
                    const TString &beamLabel,
                    double x1 = 0.55,
                    double y1 = 0.72,
                    double x2 = 0.88,
                    double y2 = 0.88)
{
    TLegend *leg = new TLegend(x1, y1, x2, y2);
    leg->SetBorderSize(0);
    leg->SetFillStyle(0);
    leg->AddEntry(h, "Truth", "l");
    leg->AddEntry((TObject*)0, lumiLabel, "");
    leg->AddEntry((TObject*)0, beamLabel, "");
    leg->Draw();
}

//============================================================
// Helper function: save a single histogram as an individual PDF
//============================================================
void SaveHistogramPDF(TH1 *h,
                      const TString &filename,
                      const TString &outputDir,
                      const TString &lumiLabel,
                      const TString &beamLabel,
                      bool useXRange = false,
                      double xMin = 0.0,
                      double xMax = 0.0,
                      bool useLogY = false)
{
    if (!h) return;

    TCanvas *c = new TCanvas(Form("c_%s", filename.Data()),
                             filename,
                             900,
                             750);

    if (useLogY)
        c->SetLogy();

    h->SetStats(0);
    h->SetLineWidth(2);

    if (useXRange)
        h->GetXaxis()->SetRangeUser(xMin, xMax);
    h->GetYaxis()->SetTitleOffset(1.4);
    h->GetXaxis()->SetTitleOffset(1.4);

    h->Draw("hist");
    AddTruthLegend(h, lumiLabel, beamLabel);

    c->SaveAs(Form("%s/%s.pdf", outputDir.Data(), filename.Data()));

    delete c;
}

//============================================================
// Main macro
//============================================================
void dvmp_lAger_level_check_individual_pdf()
{
    gStyle->SetOptStat(0);

    // ---------------- Input file ----------------
    TString inputFile =
        "lager-vmp-00mrad.jpsi-10on130.4pi.disp-jpsi-00-electron.run00001-lumi10.root";

    // ---------------- Output directory ----------------
    TString outputDir = "dvmp_jpsi_plots_April_2026";
    gSystem->mkdir(outputDir, kTRUE);

    // ---------------- Read the lAger tree ----------------
    TChain *T = new TChain("lAger");
    T->Add(inputFile);

    Long64_t Nevents = T->GetEntries();
    std::cout << "Number of events = " << Nevents << std::endl;

    if (Nevents == 0) {
        std::cout << "ERROR: No events found. Check file name and tree name." << std::endl;
        return;
    }

    // ---------------- Extract beam energy and luminosity from filename ----------------
    TString beamLabel = "Beam unknown";
    TString lumiLabel = "lAger, luminosity unknown";

    TPRegexp beamRegex("([0-9]+)on([0-9]+)");
    TObjArray *beamMatch = beamRegex.MatchS(inputFile);

    if (beamMatch && beamMatch->GetEntries() >= 3) {
        TString eBeam = ((TObjString*)beamMatch->At(1))->GetString();
        TString pBeam = ((TObjString*)beamMatch->At(2))->GetString();
        beamLabel = Form("%s#times%s GeV^{2}", eBeam.Data(), pBeam.Data());
    }

    TPRegexp lumiRegex("lumi([0-9]+)");
    TObjArray *lumiMatch = lumiRegex.MatchS(inputFile);

    if (lumiMatch && lumiMatch->GetEntries() >= 2) {
        TString lumi = ((TObjString*)lumiMatch->At(1))->GetString();
        lumiLabel = Form("lAger, %s fb^{-1}", lumi.Data());
    }

    std::cout << "Beam label = " << beamLabel << std::endl;
    std::cout << "Luminosity label = " << lumiLabel << std::endl;

    // ---------------- Branch variables ----------------
    Int_t evgen;
    Float_t W, Q2, nu, x, y, epsilon, R, t, xv;
    Short_t n_part;

    const Int_t size1 = 20;
    Double_t fPx[size1], fPy[size1], fPz[size1], fE[size1];
    Int_t PDG[size1];

    T->SetBranchAddress("evgen", &evgen);
    T->SetBranchAddress("W", &W);
    T->SetBranchAddress("Q2", &Q2);
    T->SetBranchAddress("nu", &nu);
    T->SetBranchAddress("x", &x);
    T->SetBranchAddress("y", &y);
    T->SetBranchAddress("epsilon", &epsilon);
    T->SetBranchAddress("R", &R);
    T->SetBranchAddress("t", &t);
    T->SetBranchAddress("xv", &xv);
    T->SetBranchAddress("n_part", &n_part);

    // Important: pass the array itself, not &array[n_part]
    T->SetBranchAddress("particles.fPdgCode", PDG);
    T->SetBranchAddress("particles.fPx", fPx);
    T->SetBranchAddress("particles.fPy", fPy);
    T->SetBranchAddress("particles.fPz", fPz);
    T->SetBranchAddress("particles.fE", fE);

    // ---------------- Histograms ----------------
    TH1F *Electroneta =
        new TH1F("Electroneta", ";#eta;Counts", 200, -7, 7);

    TH1F *Protoneta =
        new TH1F("Protoneta", ";#eta;Counts", 200, 4, 10);

    TH1F *Positroneta =
        new TH1F("Positroneta", ";#eta;Counts", 200, -7, 7);

    TH1F *fElectron =
        new TH1F("fElectron", ";#eta;Counts", 200, -4, 0);

    TH1F *iProtoneta =
        new TH1F("iProtoneta", ";#eta;Counts", 200, 4, 10);

    TH1F *JPsiMass1 =
        new TH1F("JPsiMass1", ";M_{e^{+}e^{-}} [GeV];Counts", 200, 2.5, 3.5);

    TH1F *tElectron =
        new TH1F("tElectron", ";P_{T} [GeV];Counts", 200, -1, 5);

    TH1F *tPositron =
        new TH1F("tPositron", ";P_{T} [GeV];Counts", 200, -1, 5);

    TH1F *tProton =
        new TH1F("tProton", ";P_{T} [GeV];Counts", 200, -1, 3);

    TH1F *tfElectron =
        new TH1F("tfElectron", ";P_{T} [GeV];Counts", 200, -1, 5);

    TH1F *lElectron =
        new TH1F("lElectron", ";P_{Z} [GeV];Counts", 200, -250, 250);

    TH1F *lPositron =
        new TH1F("lPositron", ";P_{Z} [GeV];Counts", 200, -250, 250);

    TH1F *lProton =
        new TH1F("lProton", ";P_{Z} [GeV];Counts", 200, 100, 150);

    TH1F *lfElectron =
        new TH1F("lfElectron", ";P_{Z} [GeV];Counts", 200, -25, 5);

    TH1F *tt =
        new TH1F("tt", ";|t| [GeV^{2}];Counts", 200, 0, 2);

    TH1F *QQ2 =
        new TH1F("QQ2", ";Q^{2} [GeV^{2}];Counts", 100, 1, 10);

    TH1F *xxv =
        new TH1F("xxv", ";x_{v};Counts", 200, -0.1, 1);

    // ---------------- Event loop ----------------
    for (Long64_t l = 0; l < Nevents; l++) {

        T->GetEntry(l);

        TLorentzVector decayElectron;
        TLorentzVector scatteredProton;
        TLorentzVector decayPositron;
        TLorentzVector scatteredElectron;
        TLorentzVector initialProton;

        bool hasDecayElectron     = false;
        bool hasScatteredProton   = false;
        bool hasDecayPositron     = false;
        bool hasScatteredElectron = false;
        bool hasInitialProton     = false;

        for (Int_t j = 0; j < n_part && j < size1; j++) {

            TLorentzVector p;
            p.SetPxPyPzE(fPx[j], fPy[j], fPz[j], fE[j]);

            // These indices follow the current lAger event structure.
            // Verify with PDG[j] if the particle ordering changes.
            if (j == 8) {
                decayElectron = p;
                Electroneta->Fill(p.Eta());
                hasDecayElectron = true;
            }

            if (j == 6) {
                scatteredProton = p;
                Protoneta->Fill(p.Eta());
                hasScatteredProton = true;
            }

            if (j == 7) {
                decayPositron = p;
                Positroneta->Fill(p.Eta());
                hasDecayPositron = true;
            }

            if (j == 4) {
                scatteredElectron = p;
                fElectron->Fill(p.Eta());
                hasScatteredElectron = true;
            }

            if (j == 5) {
                initialProton = p;
                iProtoneta->Fill(p.Eta());
                hasInitialProton = true;
            }
        }

        if (hasDecayElectron && hasDecayPositron) {
            TLorentzVector JPsi = decayElectron + decayPositron;
            JPsiMass1->Fill(JPsi.M());
        }

        if (hasDecayElectron) {
            tElectron->Fill(decayElectron.Pt());
            lElectron->Fill(decayElectron.Pz());
        }

        if (hasDecayPositron) {
            tPositron->Fill(decayPositron.Pt());
            lPositron->Fill(decayPositron.Pz());
        }

        if (hasScatteredProton) {
            tProton->Fill(scatteredProton.Pt());
            lProton->Fill(scatteredProton.Pz());
        }

        if (hasScatteredElectron) {
            tfElectron->Fill(scatteredElectron.Pt());
            lfElectron->Fill(scatteredElectron.Pz());
        }

        tt->Fill(-t);
        QQ2->Fill(Q2);
        xxv->Fill(xv);
    }

    //============================================================
    // Save individual histograms as separate PDF files
    //============================================================

    // Eta distributions
    SaveHistogramPDF(Electroneta, "Electron_eta", outputDir, lumiLabel, beamLabel);
    SaveHistogramPDF(Positroneta, "Positron_eta", outputDir, lumiLabel, beamLabel);
    SaveHistogramPDF(Protoneta, "ScatteredProton_eta", outputDir, lumiLabel, beamLabel,
                     true, 5.0, 11.0);
    SaveHistogramPDF(fElectron, "ScatteredElectron_eta", outputDir, lumiLabel, beamLabel,
                     true, -4.0, 0.0);
    SaveHistogramPDF(iProtoneta, "InitialProton_eta", outputDir, lumiLabel, beamLabel,
                     true, -10.0, 10.0);

    // J/psi mass
    SaveHistogramPDF(JPsiMass1, "JPsi_mass", outputDir, lumiLabel, beamLabel,
                     true, 2.8, 3.3);

    // Transverse momentum distributions
    SaveHistogramPDF(tElectron, "DecayElectron_pt", outputDir, lumiLabel, beamLabel);
    SaveHistogramPDF(tPositron, "DecayPositron_pt", outputDir, lumiLabel, beamLabel);
    SaveHistogramPDF(tProton, "ScatteredProton_pt", outputDir, lumiLabel, beamLabel);
    SaveHistogramPDF(tfElectron, "ScatteredElectron_pt", outputDir, lumiLabel, beamLabel);

    // Longitudinal momentum distributions
    SaveHistogramPDF(lElectron, "DecayElectron_pz", outputDir, lumiLabel, beamLabel,
                     true, -25.0, 60.0);
    SaveHistogramPDF(lPositron, "DecayPositron_pz", outputDir, lumiLabel, beamLabel,
                     true, -25.0, 60.0);
    SaveHistogramPDF(lProton, "ScatteredProton_pz", outputDir, lumiLabel, beamLabel,
                     true, 110.0, 150.0);
    SaveHistogramPDF(lfElectron, "ScatteredElectron_pz", outputDir, lumiLabel, beamLabel,
                     true, -25.0, 5.0);

    // Generator-level physics distributions
    SaveHistogramPDF(tt, "t_distribution", outputDir, lumiLabel, beamLabel,
                     true, 0.0, 1.7, true);
    SaveHistogramPDF(QQ2, "Q2_distribution", outputDir, lumiLabel, beamLabel,
                     true, 1.0, 10.0, true);
    SaveHistogramPDF(xxv, "xv_distribution", outputDir, lumiLabel, beamLabel,
                     true, 0.0, 1.0, true);

    //============================================================
    // Original combined canvases
    //============================================================

    // ---------------- Canvas 1: eta distributions ----------------
    TCanvas *c1 = new TCanvas("c1", "Eta distributions", 800, 800);
    c1->Divide(2, 2);

    c1->cd(1);
    Electroneta->SetStats(0);
    Electroneta->Draw("hist");
    AddTruthLegend(Electroneta, lumiLabel, beamLabel);

    c1->cd(2);
    Positroneta->SetStats(0);
    Positroneta->Draw("hist");
    AddTruthLegend(Positroneta, lumiLabel, beamLabel);

    c1->cd(3);
    Protoneta->SetStats(0);
    Protoneta->GetXaxis()->SetRangeUser(5, 11);
    Protoneta->Draw("hist");
    AddTruthLegend(Protoneta, lumiLabel, beamLabel);

    c1->cd(4);
    fElectron->SetStats(0);
    fElectron->GetXaxis()->SetRangeUser(-4, 0);
    fElectron->Draw("hist");
    AddTruthLegend(fElectron, lumiLabel, beamLabel);

    c1->SaveAs(Form("%s/Generator_level_eta_distributions.pdf", outputDir.Data()));

    // ---------------- Canvas 2: J/psi mass ----------------
    TCanvas *c2 = new TCanvas("c2", "J/psi Mass", 800, 800);

    JPsiMass1->SetStats(0);
    JPsiMass1->GetXaxis()->SetRangeUser(2.8, 3.3);
    JPsiMass1->Draw("hist");
    AddTruthLegend(JPsiMass1, lumiLabel, beamLabel);

    c2->SaveAs(Form("%s/Generator_level_jpsi_mass.pdf", outputDir.Data()));

    // ---------------- Canvas 3: initial proton eta ----------------
    TCanvas *c3 = new TCanvas("c3", "Initial Proton Eta", 800, 800);

    iProtoneta->SetStats(0);
    iProtoneta->GetXaxis()->SetRangeUser(-10, 10);
    iProtoneta->Draw("hist");
    AddTruthLegend(iProtoneta, lumiLabel, beamLabel);

    c3->SaveAs(Form("%s/Generator_level_initial_proton_eta.pdf", outputDir.Data()));

    // ---------------- Canvas 4: transverse momentum ----------------
    TCanvas *c4 = new TCanvas("c4", "Transverse Momentum", 800, 800);
    c4->Divide(2, 2);

    c4->cd(1);
    tElectron->SetStats(0);
    tElectron->Draw("hist");
    AddTruthLegend(tElectron, lumiLabel, beamLabel);

    c4->cd(2);
    tPositron->SetStats(0);
    tPositron->Draw("hist");
    AddTruthLegend(tPositron, lumiLabel, beamLabel);

    c4->cd(3);
    tProton->SetStats(0);
    tProton->Draw("hist");
    AddTruthLegend(tProton, lumiLabel, beamLabel);

    c4->cd(4);
    tfElectron->SetStats(0);
    tfElectron->Draw("hist");
    AddTruthLegend(tfElectron, lumiLabel, beamLabel);

    c4->SaveAs(Form("%s/Generator_level_transverse_momentum.pdf", outputDir.Data()));

    // ---------------- Canvas 5: longitudinal momentum ----------------
    TCanvas *c5 = new TCanvas("c5", "Longitudinal Momentum", 800, 800);
    c5->Divide(2, 2);

    c5->cd(1);
    lElectron->SetStats(0);
    lElectron->GetXaxis()->SetRangeUser(-25, 60);
    lElectron->Draw("hist");
    AddTruthLegend(lElectron, lumiLabel, beamLabel);

    c5->cd(2);
    lPositron->SetStats(0);
    lPositron->GetXaxis()->SetRangeUser(-25, 60);
    lPositron->Draw("hist");
    AddTruthLegend(lPositron, lumiLabel, beamLabel);

    c5->cd(3);
    lProton->SetStats(0);
    lProton->GetXaxis()->SetRangeUser(110, 150);
    lProton->Draw("hist");
    AddTruthLegend(lProton, lumiLabel, beamLabel);

    c5->cd(4);
    lfElectron->SetStats(0);
    lfElectron->GetXaxis()->SetRangeUser(-25, 5);
    lfElectron->Draw("hist");
    AddTruthLegend(lfElectron, lumiLabel, beamLabel);

    c5->SaveAs(Form("%s/Generator_level_longitudinal_momentum.pdf", outputDir.Data()));

    // ---------------- Canvas 6: generator-level summary distributions ----------------
    TCanvas *c6 = new TCanvas("c6", "Generator-level summary distributions", 800, 800);
    c6->Divide(2, 2);

    c6->cd(1);
    gPad->SetLogy();
    tt->SetStats(0);
    tt->SetLineWidth(2);
    tt->GetXaxis()->SetRangeUser(0.0, 1.7);
    tt->Draw("hist");
    AddTruthLegend(tt, lumiLabel, beamLabel);

    c6->cd(2);
    gPad->SetLogy();
    QQ2->SetStats(0);
    QQ2->SetLineWidth(2);
    QQ2->GetXaxis()->SetRangeUser(1.0, 10.0);
    QQ2->Draw("hist");
    AddTruthLegend(QQ2, lumiLabel, beamLabel);

    c6->cd(3);
    gPad->SetLogy();
    xxv->SetStats(0);
    xxv->SetLineWidth(2);
    xxv->GetXaxis()->SetRangeUser(0.0, 1.0);
    xxv->Draw("hist");
    AddTruthLegend(xxv, lumiLabel, beamLabel);

    c6->cd(4);
    gPad->SetLogy();
    JPsiMass1->SetStats(0);
    JPsiMass1->SetLineWidth(2);
    JPsiMass1->GetXaxis()->SetRangeUser(2.8, 3.3);
    JPsiMass1->Draw("hist");
    AddTruthLegend(JPsiMass1, lumiLabel, beamLabel);

    c6->SaveAs(Form("%s/Generator_level_summary_distributions.pdf", outputDir.Data()));

    c6->Update();

    std::cout << "\nSaved combined and individual PDF plots in: "
              << outputDir << std::endl;
}
